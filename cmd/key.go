...
var keyCmd = &cobra.Command{
  ...
  step-kms-openbao-plugin key 'azurekms:vault=my-key-vault;name=my-key'

  # Get the public key key from AWS KMS.
  step-kms-openbao-plugin key 'awskms:key-id=acbebc8f-822d-4c1c-b5d1-eb3a8fcaced7;region=us-west-1'

  # Get key from a YubiKey:
  step-kms-openbao-plugin key yubikey:slot-id=82

  # Get a key from the ssh-agent:
  step-kms-openbao-plugin key sshagentkms:user@localhost

  # Get a key from the default TPM KMS with KMS URI:
  step-kms-openbao-plugin key my-key --kms tpmkms

  # Get a key from the a TPM KMS, backed by /tmp/tpmobjects with KMS URI:
  step-kms-openbao-plugin key my-key --kms tpmkms:storage-directory=/tmp/tpmobjects

  # Get a key from the default TPM KMS:
  step-kms-openbao-plugin key tpmkms:name=my-key

  # Get a key from the TSS2 PEM file:
  step-kms-openbao-plugin key tpmkms:path=tss2.pem

  # Get an AK public key from the default TPM KMS:
  step-kms-openbao-plugin key 'tpmkms:name=my-ak;ak=true'`,
