...
var rootCmd = &cobra.Command{
	Use:   "step-kms-openbao-plugin",
	Short: "step plugin to manage KMSs (OpenBao Edition)",
	Long: `step-kms-openbao-plugin is a plugin for step that allows performing cryptographic
operations on Hardware Security Modules (HSMs), Cloud Key Management Services
(KMSs), devices like YubiKey that implement a Personal Identity Verification (PIV)
interface, and secret management systems like OpenBao (or HashiCorp Vault).
This command uses the term KMS to refer to any of these interfaces.

step-kms-openbao-plugin can be used using 'step kms [command]', or as a standalone
application.
...
Common operations include:
 ...
 - Use OpenBao Transit secrets engine for key operations`,
}
